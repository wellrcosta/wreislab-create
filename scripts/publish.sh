...
pnpm exec tsc --noEmit || fail "TypeScript errors found — fix them before publishing"
ok "TypeScript OK"
...
step "3/7 Verify shebang"
SHEBANG=$(head -1 "$REPO_DIR/dist/cli.js")
if [[ "$SHEBANG" != "#!/usr/bin/env node" ]]; then
  fail "Shebang missing in dist/cli.js — the file will not be executable via npx"
fi
ok "Shebang present: $SHEBANG"

# ── 4. Pack validation ────────────────────────────────────────────────────────
step "4/7 Validate pack contents"
PACK_OUT=$(npm pack --dry-run 2>&1)

# Must contain dist/
echo "$PACK_OUT" | grep -q "dist/" || fail "dist/ is not in the package — check the 'files' field in package.json"

# Must NOT contain src/ (except src in dist paths like dist/templates/...)
if echo "$PACK_OUT" | grep -v "dist/" | grep -q " src/"; then
  fail "src/ is being published — remove it from the 'files' field or add it to .npmignore"
fi
...
if echo "$PACK_OUT" | grep -qE "\.env$|\.env\.(development|production|local|staging)|credentials|secrets"; then
  fail "Sensitive files detected in the package — review before publishing"
fi

PACK_SIZE=$(echo "$PACK_OUT" | grep "package size:" | awk '{print $3, $4}' || echo "unknown")
ok "Pack OK${PACK_SIZE:+ (size: $PACK_SIZE)}"

# ── 5. Dependency audit (warning only) ───────────────────────────────────────
step "5/7 Dependency audit"
if bash "$SCRIPT_DIR/check-deps.sh" 2>/dev/null; then
  ok "All dependencies are up to date"
else
  warn "Some dependencies have updates available — consider updating before publishing"
fi

# ── 6. Version bump ───────────────────────────────────────────────────────────
step "6/7 Version bump"
...
echo "    [0] skip   — keep current version"
echo ""
read -r -p "  Option (0-3): " BUMP_CHOICE
...
case "$BUMP_CHOICE" in
  ...
  3) BUMP_TYPE="major" ;;
  0) BUMP_TYPE="" ;;
  *) fail "Invalid option: $BUMP_CHOICE" ;;
esac
...
# ── 7. Publish ────────────────────────────────────────────────────────────────
step "7/7 Publish to npm"
if [[ "$DRY_RUN" == "true" ]]; then
  warn "DRY RUN — publish skipped"
  warn "Command that would run: pnpm publish --access public --no-git-checks --otp=\$OTP"
else
  echo -e "  Publishing ${BOLD}wreislab-create@${NEW_VERSION}${RESET}..."
  OTP_FLAG=""
  [[ -n "$OTP" ]] && OTP_FLAG="--otp=$OTP"
  pnpm publish --access public --no-git-checks $OTP_FLAG || fail "Publish failed"
  ok "Published: wreislab-create@${NEW_VERSION}"
fi
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
  ...
  if [[ "$DRY_RUN" != "true" ]]; then
    ...
    ok "Commit and tag v${NEW_VERSION} pushed to GitHub"
  else
    warn "DRY RUN — git push skipped"
  fi
fi

echo ""
echo -e "${GREEN}${BOLD}✓ Pipeline complete — wreislab-create@${NEW_VERSION}${RESET}"
[[ "$DRY_RUN" == "true" ]] && echo -e "${YELLOW}  (dry run — nothing was published)${RESET}"
